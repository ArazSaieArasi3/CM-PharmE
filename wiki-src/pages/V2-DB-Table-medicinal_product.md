...
## Purpose
Canonical medicinal-product identity with a bounded primary-substance hook.

## Keys and structure
- **Primary key:** `medicinal_product_id`
- **Foreign keys:** 1
...
| `primary_substance_id` | `BIGINT` | Yes | FK → pharmaceutical_substance.substance_id | — |
...
|---|---|---|---|---|
| `https://w3id.org/cm-pharme/2.0/MedicinalProduct` | class | direct | `medicinal_product_id/public_id` | Product identity layer |
| `https://w3id.org/cm-pharme/2.0/hasActiveSubstance` | object_property | bounded | `primary_substance_id` | W6 reference schema models one primary substance hook; richer composition may require extension |

See [[V2 Ontology Reference]] for semantic definitions. Mapping authority remains `v2/data/mappings/ontology-rdb-mapping.csv`.

## Common joins
- `medicinal_product.primary_substance_id` → `pharmaceutical_substance.substance_id`

## Safe synthetic row fragment
`{"medicinal_product_id": "1001", "public_id": "syn-medicinal-product-001", "preferred_name": "synthetic-preferred-name", "primary_substance_id": "1001", "ontology_iri": "https://w3id.org/cm-pharme/2.0/Example"}`
